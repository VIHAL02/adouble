...
class HierarchicalFoodPredictor:
    ...
    def predict(self, event_time, meal_type, event_type, guest_count, selected_items):
        ...
        # Identify main items in the menu
        ...
        for category, unit_groups in special_category_items.items():
            for unit_type, items in unit_groups.items():
                ...
                special_params = {
                    ...
                    "has_main_item": main_items_info["has_main_item"],
                    "main_item_category": main_items_info.get("primary_main_category")
                }
        ...
        # Apply main item adjustments for both the main item itself and complementary items
        if main_items_info["has_main_item"] and main_items_info.get("primary_main_category"):
            ...
            if primary_main == "Rice" and "Rice" in category_quantities:
                ...
                if "Biryani" not in items_by_category and "Flavored_Rice" not in items_by_category:
                    ...
                    logger.info(f"Main item adjustment: Increased Rice quantity as primary main item ({orig_qty:.2f} → {adjusted_qty:.2f})")
                    # Add debug log
                    logger.info(f"After adjustment, Rice quantity in category_quantities is now: {category_quantities['Rice']['value']:.2f}")
                    # Directly update category_per_person as well
                    if "Rice" in category_per_person:
                        category_per_person["Rice"] = adjusted_qty
                        logger.info(f"Updated category_per_person['Rice'] to {adjusted_qty:.2f}")
                ...
                if "Rice" not in items_by_category and "Flavored_Rice" not in items_by_category:
                    ...
                    logger.info(f"Main item adjustment: Increased Biryani quantity as primary main item ({orig_qty:.2f} → {adjusted_qty:.2f})")
                    # Add debug log
                    logger.info(f"After adjustment, Biryani quantity in category_quantities is now: {category_quantities['Biryani']['value']:.2f}")
                    # Directly update category_per_person as well
                    if "Biryani" in category_per_person:
                        category_per_person["Biryani"] = adjusted_qty
                        logger.info(f"Updated category_per_person['Biryani'] to {adjusted_qty:.2f}")
                ...
                if "Rice" not in items_by_category and "Biryani" not in items_by_category:
                    ...
                    logger.info(f"Main item adjustment: Increased Flavored Rice quantity as primary main item ({orig_qty:.2f} → {adjusted_qty:.2f})")
                    # Add debug log
                    logger.info(f"After adjustment, Flavored_Rice quantity in category_quantities is now: {category_quantities['Flavored_Rice']['value']:.2f}")
                    # Directly update category_per_person as well
                    if "Flavored_Rice" in category_per_person:
                        category_per_person["Flavored_Rice"] = adjusted_qty
                        logger.info(f"Updated category_per_person['Flavored_Rice'] to {adjusted_qty:.2f}")
            
            # THEN adjust quantities for items that complement the main item
            for category in category_quantities:
                # Skip special categories and the main category itself
                if category in special_categories or category == primary_main:
                    continue
                ...
                if primary_main == "Biryani":
                    ...
                    if category == "Raitha":
                        ...
                        category_quantities[category]["value"] = adjusted_qty
                        logger.info(f"Main item adjustment: Increased {category} quantity for Biryani ({orig_qty:.2f} → {adjusted_qty:.2f})")
                    elif category == "Salan":
                        orig_qty = category_quantities[category]["value"]
                        ...
                        category_quantities[category]["value"] = adjusted_qty
                        logger.info(f"Main item adjustment: Reduced {category} quantity due to Biryani ({orig_qty:.2f} → {adjusted_qty:.2f})")
                    ...
                    if category == "Rice":
                        ...
                        adjusted_qty = orig_qty * 0.7
                        category_quantities[category]["value"] = adjusted_qty
                        logger.info(f"Main item adjustment: Reduced {category} quantity due to Flavored Rice ({orig_qty:.2f} → {adjusted_qty:.2f})")
                    elif category == "Curries":
                        # Might want more curry with flavored rice
                        orig_qty = category_quantities[category]["value"]
                        adjusted_qty = orig_qty * 1.1
                        category_quantities[category]["value"] = adjusted_qty
                        logger.info(f"Main item adjustment: Increased {category} quantity for Flavored Rice ({orig_qty:.2f} → {adjusted_qty:.2f})")
                    ...
                    if category == "Curries":
                        ...
                        adjusted_qty = orig_qty * 1.2
                        category_quantities[category]["value"] = adjusted_qty
                        logger.info(f"Main item adjustment: Increased {category} quantity for Rice ({orig_qty:.2f} → {adjusted_qty:.2f})")
                    elif category == "Dal":
        ...
        # Sync adjusted values from category_quantities back to category_per_person
        for category, data in category_quantities.items():
            if category in category_per_person and not isinstance(category_per_person[category], dict):
                # Update category_per_person with the final adjusted values
                category_per_person[category] = data["value"]
                logger.info(f"Syncing {category} adjusted quantity from {category_per_person[category]:.2f} to {data['value']:.2f}")
...
def load_model(filename="adouble1.dill"):
...
if __name__ == "__main__":
    ...
    save_model(predictor, "adouble1.dill")
    
    # Optionally, test loading and using the model
    loaded_predictor = load_model("adouble1.dill")
